...
template <class T> ostream& operator<<(ostream& out, VECTOR2<T> v)
{
    out<<'('<<v.x<<','<<v.y<<')';
    return out;
}
template <class T> T dx(VECTOR2<T> v1, VECTOR2<T> v2)
{
    return v2.getx() - v1.getx();
}
template <class T> T dy(VECTOR2<T> v1, VECTOR2<T> v2)
{
    return v2.gety() - v1.gety();
}
template <class T> double angle_bn(VECTOR2<T> v1, VECTOR2<T> v2)
{
    T y = dy(v1, v2);
    T x = dx(v1, v2);
    double rang = (x ? (180 / 3.14) * atan((float)(y/x)) : (y > 0 ? 90 : -90));
    bool xl = (x < 0);
    bool yl = (y < 0);
...
template <class T> double angler(VECTOR2<T> v1, VECTOR2<T> v2)
{
    T y = dy(v1, v2);
    T x = dx(v1, v2);
    double rang = (x ? (180 / 3.14) * atan((float)(-1 * y)/(float)x) : (y > 0 ? -90: 90));
    bool xl = (x < 0);
    bool yl = (y > 0);
    if(xl && yl){rang += 180;return rang;}
    else if(xl){rang = 180 - (-1 * (rang)); return rang;}
...
template <class T> VECTOR2<T> C_x_y_find(vector<VECTOR2<T>>vertices)
{
    VECTOR2<T> C_x_y;
    T Area = 0;
    T cx= 0, cy = 0;
...
template <class T> bool in_RECT(VECTOR2<T>* vecs, VECTOR2<T> point, int size)
{
    //implement
    bool xcr = false;
    bool ycr = false;
...
template <class T> VECTOR2<T> intersection(VECTOR2<T> vec11, VECTOR2<T> vec12, VECTOR2<T> vec21, VECTOR2<T> vec22)
{
    double m1 = (double)((1 * dy(vec11, vec12))/dx(vec11, vec12));
    double m2 = (double)((1 * dy(vec21, vec22))/dx(vec21, vec22));
...
template <class T> bool bound_r(VECTOR2<T> v11, VECTOR2<T> v12, VECTOR2<T> vec)
{
    if(v11 == v12)
    {
        return vec == v11;
    ...
    else
    {
        ...
        else
        {
            if
            (
                (angler(v11, v12) == angler(v11, vec)) || (v11 == vec)
                ||
                (angler(v12, v11) == angler(v12, vec)) || (v12 == vec)
            )
            {
                return
                (
...
template <class T> bool bound_rL(VECTOR2<T> v11, VECTOR2<T> v12, VECTOR2<T> vec)
{
    if(v11 == v12)
    {
        return vec == v11;
...
template <class T> VECTOR2<T> intersector(VECTOR2<T> vec11, VECTOR2<T> vec12, VECTOR2<T> vec21, VECTOR2<T> vec22)
{
    double m1 = (double)(((double)(-1 * dy(vec11, vec12)))/(double)(dx(vec11, vec12)));
    double m2 = (double)(((double)(-1 * dy(vec21, vec22)))/(double)(dx(vec21, vec22)));
...
template <class T> bool intersector_B(VECTOR2<T> vec11, VECTOR2<T> vec12, VECTOR2<T> vec21, VECTOR2<T> vec22)
{
    bool l1 = bound_rL(vec11, vec12, intersector(vec11, vec12, vec21, vec22));
    bool l2 = bound_rL(vec21, vec22, intersector(vec11, vec12, vec21, vec22));
    return l1 && l2;
}
template <class T> bool in_polyB(VECTOR2<T>* vecs, VECTOR2<T> point, VECTOR2<T> term, int size)
{
    int c = 0;
    int count = 0;
...
template <class T> bool p_poly(VECTOR2<T>* vecs, VECTOR2<T> point, int size)
{
    return in_polyB(vecs, point, point + VECTOR2<T>(2000, 0), size);
}
template <class T> bool in_polyB(vector<VECTOR2<T>> vecs, VECTOR2<T> point, VECTOR2<T> term)
{
    int c = 0;
    int count = 0;
    ...
    for(int i = 0; i < vecs.size(); i++)
    {
        ...
        {
            c++;
            //cout<<" vec["<<i<<"] & vec["<<(i != vecs.size()-1 ? i + 1 : 0)<<"]: "<<intersector(point, term, vecs[i], vecs[i != vecs.size()-1 ? i + 1: 0])<<endl;
            //cout<<" checking intersection point similarities of:\n";
            //cout<<" vertices: "<<i<<(i != vecs.vecs.size()()-1 ? i + 1 : 0)<<intersector(point, term, vecs[i], vecs[i != vecs.size()-1 ? i + 1: 0])<<" and "<<(i != vecs.size()-1? i + 1 : 0)<<(i < vecs.size()-2 ? i + 2: (i == vecs.size()-2 ? 0 : 1))<<intersector(point, term, vecs[i != vecs.size()-1 ? i + 1: 0], vecs[i < vecs.size()-2 ? i + 2: (i == vecs.size()-2 ? 0 : 1)])<<endl;
            if((intersector(point, term, vecs[i], vecs[i != vecs.size()-1 ? i + 1: 0]) - intersector(point, term, vecs[i != vecs.size()-1 ? i + 1: 0], vecs[i < vecs.size()-2 ? i + 2: (i == vecs.size()-2 ? 0 : 1)])).getmag() > 10)
            {
          //      cout<<" approved\n";
                count++;
            }
    ...
    if(count == 1)
    {
        ...
        for(int i = 0; i < vecs.size(); i++)
        {
            ...
            {
                s++;
                //cout<<" vec["<<i<<"] & vec["<<(i != vecs.size()-1 ? i + 1 : 0)<<"]: "<<intersector(point, term, vecs[i], vecs[i != size-1 ? i + 1: 0])<<endl;
                //cout<<" checking intersection point similarities of:\n";
                //cout<<" vertices: "<<i<<(i != size-1 ? i + 1 : 0)<<intersector(point, term, vecs[i], vecs[i != vecs.size()-1 ? i + 1: 0])<<" and "<<(i != vecs.size()-1? i + 1 : 0)<<(i < vecs.size()-2 ? i + 2: (i == vecs.size()-2 ? 0 : 1))<<intersector(point, term, vecs[i != vecs.size()-1 ? i + 1: 0], vecs[i < vecs.size()-2 ? i + 2: (i == vecs.size()-2 ? 0 : 1)])<<endl;
                if((intersector(point, term, vecs[i], vecs[i != vecs.size()-1 ? i + 1: 0]) - intersector(point, term, vecs[i != vecs.size()-1 ? i + 1: 0], vecs[i < vecs.size()-2 ? i + 2: (i == vecs.size()-2 ? 0 : 1)])).getmag() > 10)
                {
                    //cout<<" approved\n";
                    scount++;
...
template <class T> void rotate_r(VECTOR2<T> center, VECTOR2<T>& rotated, double angle)
{
    //cout<<" rotating vector"<<rotated<<" on "<<center<<" by "<<angle<<" degrees\n";
    T dist = (center - rotated).getmag();
    double deg = angle;
    double rad = torad(deg);
    double rdeg = angler(center, rotated);
...
void rotate_r(Vint2 center, Vint2& rotated, double angle)
{
    //cout<<" rotating vector"<<rotated<<" on "<<center<<" by "<<angle<<" degrees\n";
    //cout<<" calling rotate_r from integer: \n";
    int dist = (center - rotated).getmag();
    double deg = angle;
    double rad = torad(deg);
    double rdeg = angler(center, rotated);
    double rrad = torad(rdeg);
    ...
    //cout<<" yadd = "<<(double)dist<<" * "<<sin(rrad)<<" = "<<yadd<<endl;
    //cout<<" sqrt(xadd * xadd + yadd * yadd) = "<<sqrt((xadd * xadd) + (yadd * yadd))<<endl;
    rotated.getx() = center.getx() + (int)xadd;
    rotated.gety() = center.gety() - (int)yadd;
    //if((center - rotated).getmag() != dist)cout<<" distance error\n";
}
template <class T> VECTOR2<T> ret_rotate_r(VECTOR2<T> center, VECTOR2<T> rotated, double angle)
{
