...
TARGET := $(TEST_BINS)
TARGET_MAIN := UI2cont_test

.SECONDARY: $(SDL_OBJS) $(GUI_OBJS) $(TEST_OBJS)
# Default target
.PHONY: all
#all: $(TEST_BINS)
...
$(BIN_DIR)/$(TARGET_MAIN): $(TEST_OBJ)/$(TARGET_MAIN).o $(SDL_OBJS) $(BAS_OBJS) $(GUI_OBJS) | $(BIN_DIR)
	@echo "Linking $@"
	$(CXX) $^ -o $@ $(LDFLAGS)
...
#Rule for core objects	
$(GUI_OBJ)/%.o: $(GUI_SRC)/%.cpp | $(GUI_OBJ)
	@echo "Compiling core/GUI $<"
	$(CXX) $(CXXFLAGS) $(CPPFLAGS) -c $< -o $@ 
