...
{
    SDL_Texture* board;
    std::cout << " Welcome to the texture mapper: \n";
    std::cout << " You have got the foolowing graphics drivers: \n";
    ...
    int chosenDriver;
    std::cout << " Enter a number to choose a driver (-1 - 5) (-1 for default): ";
    std::cin >> chosenDriver;
    system("cls");
    int rendermode;
    std::cout << " SDL_RENDERER_TARGETTEXTURE - 8\n";
    std::cout << " SDL_RENDERER_SOFTWARE - 1\n";
    std::cout << " SDL_RENDERER_PRESENTVSYNC - 4\n";
    std::cout << " SDL_RENDERER_ACCELERATED - 2\n";
    std::cout << " Choose out of the following rendering modes: ";
    std::cin >> rendermode;
    system("cls");
    int blendmode;
    std::cout << " SDL_BLENDMODE_NONE - 0\n";
    std::cout << " SDL_BLENDMODE_BLEND - 1\n";
    std::cout << " SDL_BLENDMODE_ADD - 2\n";
    std::cout << " SDL_BLENDMODE_MOD - 4\n";
    std::cout << " SDL_BLENDMODE_MUL - 8\n";
    std::cout << " SDL_BLENDMODE_INVALID - 9\n";
    std::cout << " Choose blendmode: ";
    std::cin >> blendmode;
    system("cls");
    ...
    SDL_FPoint center = {win.getw()/2, win.geth()/2};
    double angle = 45;
    float distance = 200;
    ...
    while(event.type != SDL_QUIT){
        ...
        TextBox driverName(rendererInfo.name, FONT());//(, win.getren());
        driverName.setboxpos(0, 0);
        driverName.draw(win.getren(), board);
        if (!texture) {
            std::cerr << "Error creating texture: " << SDL_GetError() << std::endl;
        }
        SDL_SetTextureBlendMode(texture, (SDL_BlendMode)blendmode);
        // Top-left vertex
        vertices[0].position.x = center.x - w/2;
        vertices[0].position.y = center.y - h/2;
        vertices[0].tex_coord.x = 0.0;
        vertices[0].tex_coord.y = 0.0;
        // Top-right vertex
        vertices[1].position.x = center.x + w/2;
        vertices[1].position.y = center.y - h/2;
        vertices[1].tex_coord.x = 1.0;
        vertices[1].tex_coord.y = 0.0;
        // Bottom-right vertex
        vertices[2].position.x = center.x + w/2;
        vertices[2].position.y = center.y + h/2;
        vertices[2].tex_coord.x = 1.0;
        vertices[2].tex_coord.y = 1.0;
        // Bottom-left vertex
        vertices[3].position.x = center.x - w/2;
        vertices[3].position.y = center.y + h/2;
        vertices[3].tex_coord.x = 0.0;
        vertices[3].tex_coord.y = 1.0;
        // Center vertex
        vertices[4].position.x = center.x;
        vertices[4].position.y = center.y;
        vertices[4].tex_coord.x = 0.5;
        vertices[4].tex_coord.y = 0.5;
        SDL_Vertex vertices2[5];
        for(int i = 0; i < 5; i++){
            float z = (vertices[i].position.x - center.x) * sinf(to_rad(angle));
            float scale = distance / (distance - z);
            vertices2[i].tex_coord = vertices[i].tex_coord;
            vertices2[i].position.x = (vertices[i].position.x - center.x) * cosf(to_rad(angle)) + center.x;
            vertices2[i].position.y = (vertices[i].position.y - center.y) * scale + center.y;
        }
