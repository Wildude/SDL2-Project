...
bool NetworkManager::waitForConnection(Uint16 port) {
    ...
    if (SDLNet_ResolveHost(&ip, NULL, port) < 0) {
        std::cout << "Failed to resolve host for server: " << SDLNet_GetError() << std::endl;
        return false;
    }
...
std::string NetworkManager::getIP(TCPsocket sock) {
    ...
    if (!remoteIP) {
        std::cout << "Failed to get peer address: " << SDLNet_GetError() << std::endl;
        return "";
    }
...
bool NetworkManager::createUDP(Uint16 port) {
    ...
    if (!udpSocket) {
        std::cout << "Failed to open UDP socket: " << SDLNet_GetError() << std::endl;
        return false;
    }
    return true;
}
...
bool NetworkManager::bindUDP(const std::string& host, Uint16 port) {
    if (SDLNet_ResolveHost(&udpIP, host.c_str(), port) < 0) {
        std::cout << "Failed to resolve UDP host: " << SDLNet_GetError() << std::endl;
        return false;
    }
    return true;
}
...
bool NetworkManager::sendUDP(const void* data, int len) {
    ...
    if (SDLNet_UDP_Send(udpSocket, -1, &packet) == 0) {
        std::cout << "Failed to send UDP packet: " << SDLNet_GetError() << std::endl;
        return false;
    }
    return true;
}
...
int NetworkManager::receiveUDP(void* buffer, int maxlen) {
    ...
    if (SDLNet_UDP_Recv(udpSocket, &packet) == 0) {
        std::cout << "No UDP packet received." << std::endl;
        return 0;
    }
    return packet.len;
}
